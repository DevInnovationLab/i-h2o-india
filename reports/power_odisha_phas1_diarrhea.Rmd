...
3) Relevant descriptive statistics from Pickering et. al. (2019, The Lancet)
...
storesubsiid <- df.sim.iid |> group_by(clusize, MDE, ICC) |> do(closepowerfinder(.,.80))
#storesubs <- storesubs |> filter(MDE == .2)
storesubsiid$power <- round(storesubsiid$power, 2)
storesubsiid$diarrhea_pc <- .05
names(storesubsiid) <- c("villages_needed", "MDE", "avg_clustersize",  "power", "ICC", "diarrhea_pc")
storesubsiid <- storesubsiid[, c("avg_clustersize", "diarrhea_pc", "MDE", "ICC", "villages_needed", "power")]

storesubs20 <- storesubsiid |> filter(MDE == .2)
storesubs20 |># group_by(MDE) |> 
  knitr::kable(caption = "Number of villages needed for different scenarios. Effectsize 20%", digits = 2)
```
...
storesubs10 |># group_by(MDE) |> 
  knitr::kable(caption = "Number of villages needed for different scenarios. Effectsize 10%", digits = 2)
...
storesubsiid <- df.sim.AR1 |> group_by(clusize, MDE, ICC) |> do(closepowerfinder(.,.80))
#storesubs <- storesubs |> filter(MDE == .2)
storesubsiid$power <- round(storesubsiid$power, 2)
storesubsiid$diarrhea_pc <- .05
names(storesubsiid) <- c("villages_needed", "MDE", "avg_clustersize",  "power", "ICC", "diarrhea_pc")
...
storesubs20 <- storesubsiid |> filter(MDE == .2)
storesubs20 |># group_by(MDE) |> 
  knitr::kable(caption = "Number of villages needed for different scenarios. Effectsize 20%")
```
...
storesubs10 <- storesubsiid |> filter(MDE == .1)
storesubs10 |># group_by(MDE) |> 
  knitr::kable(caption = "Number of villages needed for different scenarios. Effectsize 10%")
