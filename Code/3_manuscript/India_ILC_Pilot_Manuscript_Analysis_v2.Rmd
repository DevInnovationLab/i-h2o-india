...
cen <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_1_baseline_census.csv"))
bl <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_1_baseline_survey.csv"))
el <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_8_endline_census.csv"))
idexx_tab <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_10_idexx_all_rounds.csv"))
idexx_abr <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_10_idexx_abr.csv"))
all_rounds <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_8_surveys_all_rounds.csv"))
mon_summary <- read_csv(paste0(user_path(), "3_final/manuscript_datasets/", "1_11_weekly_monitoring.csv"))
...
idexx_bl_tap <- idexx_tab%>%
  filter(data_round == "BL")%>% #Filtering for baseline samples only
...
idexx_bl_stored <- idexx_tab%>%
  filter(data_round == "BL")%>% #Filtering for baseline samples only
  filter(sample_type == "Stored")%>% #Filtering for stored samples only
  dplyr::select(sample_ID, assignment, village, block, panchayat_village, cf_pa_binary, ec_pa_binary, cf_log, ec_log)%>%
...
#Separating out follow-up rounds and baseline round
idexx_fu <- idexx_tab%>%
...
idexx_abr_fu <- idexx_abr%>%
  filter(data_round == "R3" | data_round == "R6") #ABR tests were only conducted in Round 3 and Round 6
...
idexx_model <- idexx_fu_tap #Converting to numeric type
...
idexx_model <- idexx_fu_stored #Converting to numeric type
...
outcome_vars <- c("prim_source_jjm", "sec_source",
                  "jjm_drinking", "water_treat_binary", "tap_issues_taste", "time_spent_treat_5")
...
p_forestplot <- ggplot(el_models_forestplot, aes(x = mean, y = model_name)) +
  ...
  scale_x_continuous(limits = c(0.0, 1.90), breaks = c(0, 0.25, 0.5, 0.75, 1.00, 1.2))+
