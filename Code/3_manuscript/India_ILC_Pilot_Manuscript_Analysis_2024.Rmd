...
ggsave(filename = "manuscript_ecoli_time_series_v1.jpeg", path = (paste0(github_path(), "/3_manuscript/figures")), dpi = 1080, width = 6, height = 3, units = "in", scale = 2)
...
p_ec_cl_4 <- idexx_cl_comb%>%
  ...
  geom_point(aes(x = test_week, y = cl_presence), color = "black", size = 3)+
  #geom_path(aes(x = test_week, y = cl_presence), color = "#FF474C")+ #Geom path makes the plot cluttered
  geom_vline(aes(xintercept = ymd("2024-02-01")), lty = 2, color = "#FF474C")+ #Placing intervention start date
  geom_text(aes(x = ymd("2024-02-01"), y = 80, label = "Intervention Start"),
            nudge_x = -25, color = "#FF474C")+
  ...
  scale_fill_brewer(palette = "Pastel1")+
  labs(x = "Date", 
       y = '% of tap samples positive for free chlorine (> 0.1 mg/L)',
       fill = "Assignment",
       title = "",
       caption = ""
       #title = expression(paste(italic("E. coli"), " r3 - Presence in Tap and Stored Drinking Water in Odisha (N = 166)"))
...
ggsave(filename = "manuscript_ecoli_chlorine_time_series_v2.jpeg", path = (paste0(github_path(), "/3_manuscript/figures")), dpi = 1080, width = 6, height = 1.5, units = "in", scale = 2)
...
idexx_abr_poisson <- idexx_abr_poisson%>%
  ...
  filter(model_name != "ec_tap")%>%
  mutate(model_name = ifelse(model_name == "ec_stored","Presence Antibiotic Resistant E. coli in Stored Water", NA))%>%
  select(!(sample_type))
...
el_forestplot <- el_models_forestplot%>%
  ...
  fp_add_lines(h_3 = gpar(lty = 1), 
               h_14 = gpar(lwd = 1, columns = 1:5, col = "black"))%>% 
  fp_set_style(box = "black",
               line = "black",
               summary = "black",
               align = "lrrr",
