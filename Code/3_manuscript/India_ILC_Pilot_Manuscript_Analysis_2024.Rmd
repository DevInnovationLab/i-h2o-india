...
p_forestplot <- ggplot(el_models_forestplot, aes(x = mean, y = model_name)) +
  ...
  scale_x_continuous(limits = c(0.5, 1.70), breaks = c(0.50, 0.75, 1.00, 1.2))+
...
p_forestplot <- p_forestplot + 
  geom_text(aes(label = estimate, x = max(Upper_CI) + 0.25), hjust = 0, size = 3) + 
    geom_text(aes(label = p_value, x = max(Upper_CI) + 0.45), hjust = 0, size = 3)

#Viewing
p_forestplot

ggsave(filename = "forestplot_v2.png", path = (paste0(github_path(), "/3_manuscript/figures")), dpi = 1080, width = 10, height = 6, units = "in")
