...
p_ec_bl <- ggplot(data = all_stats_bl) + 
  geom_col(aes(x = sample_type, y = `% Positive for E. coli`), position = "dodge") +
...
#Plotting treatment/control stats on a bar plot
p_ec_abr_combined <- ggplot(data = tc_stats_abr_pooled) + 
  geom_col(aes(x = sample_type, y = `% Positive for E. coli`, fill = assignment), position = "dodge") +
  geom_errorbar(aes(x = sample_type, ymin = `Lower CI - EC`, ymax = `Upper CI - EC`,
                    fill = assignment), position = position_dodge(width = 0.9),
                width = 0.25) +
  ...
  labs(x = "Sample Type", 
       y = expression(paste("% positive for ABR ",italic("E. coli"))),
       fill = "Assignment",
       title = expression(paste("Presence of Antibiotic Resistant ", italic("E. coli"), " in Household Drinking Water (N = 267)")),
       caption = "Stored and tap water samples were pooled across 2 monthly household surveys rounds across 20 villages in April 2024 and October 2024."
       #title = expression(paste(italic("E. coli"), " r3 - Presence in Tap and Stored Drinking Water in Odisha (N = 166)"))
       )+
  #facet_wrap(~ data_round)+
  theme_bw() + 
...
ggsave(filename = "manuscript_ABR_ecoli_pooled_results.jpeg", path = (paste0(github_path(), "/3_manuscript/figures")), dpi = 1080, width = 4, height = 2, units = "in", scale = 2.6)
