...
idexx_cl_comb_1 <- mon_summary_weekly%>%
  ...
  mutate(`Upper CI - EC` = NA) #Adding in variables from the other dataset so they can be directly combined
...
idexx_cl_comb_2 <- tc_stats_round%>%
  ...
  select(test_week, assignment, sample_type, `% Positive for E. coli`,
         `Lower CI - EC`, `Upper CI - EC`)%>%
  filter(sample_type == "Tap")%>%
  mutate(avg_concentration = NA,
         lower_ci = NA,
         upper_ci = NA,
         cl_presence = NA,
         lower_ci_pa = NA,
...
outcome_vars <- c("prim_source_jjm", "sec_source", "jjm_drinking", "water_treat_binary", "tap_issues_taste", "time_spent_treat")
