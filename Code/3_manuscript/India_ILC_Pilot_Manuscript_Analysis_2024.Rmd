...
```{r New Baseline Census Desc Stats Code}
...
binary_categorical_vars <- c("hhhead_gender", "read_write_1", 
...
binary_categorical_summary_cen <- cen %>%
  ...
  summarise(
    ...
    formatted_stat = sprintf("%.1f%% (%.2f)", p * 100, se),
    .groups = "drop"
  ) %>%
...
```{r New Baseline Survey Desc Stats}
...
cen_vars <- c("prim_source_jjm", "sec_source",
                  "jjm_drinking", "water_treat_binary", 
...
bl_vars <- c("treat_time_5min") #But can I use this to control for baseline data if the dataset is partial?
