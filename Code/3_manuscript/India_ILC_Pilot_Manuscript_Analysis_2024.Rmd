...
#Village Details
village_details_old <- read_xlsx(paste0(user_path(),"/5_lab data/India_ILC_Pilot_Rayagada_Village_Tracking.xlsx"))
...
#x: overall dataset to pull from
#var: vector containing binary outcome variable names in each dataset to model
ilc_glm <- function(x, outcomes_variables){
  lapply(outcome_variables,
         function(var){
           ...
           cluster_vcov <- vcovCL(model, cluster = x$village)
           clustered_se <- sqrt(diag(cluster_vcov))
           ...
           list(model = model,
                ...
                clustered_results = clustered_results) #Tidying models
         }
  )
  }
...
all_models <- lapply(outcome_vars,
           ...
           formula <- as.formula(paste(var, "~ assignment + block + panchayat_village"))
           model <- glm(formula, data = bl_tab, family = binomial)
           # Adjust for clustering at the village level
           cluster_vcov <- vcovCL(model, cluster = bl_tab$village)
           clustered_se <- sqrt(diag(cluster_vcov))
           # Get the coefficients with clustered standard errors
           clustered_results <- coeftest(model, vcov. = cluster_vcov)
           #Tidying models -- need to understand how to use clustered results option
...
#Need to select for treatment assignment to condense the table before this point

#Forcing to be gt objects
gt_tables <- lapply(all_models,
                    function(model_info) {
                      as_gt(model_info$table)
})

#Naming gt tables
names(gt_tables) <- outcome_vars
...
idexx_tab <- idexx%>%
  dplyr::select(assignment, village, block, panchayat_village, sample_ID, bag_ID_tap, bag_ID_stored, cf_pa_binary, sample_type, ec_pa_binary, cf_log, ec_log)%>%
  mutate(data_round = "BL")%>%
  mutate(pooled_round = "BL")
  

idexx_r1_tab <- idexx_r1%>%
  dplyr::select(assignment, village, block, panchayat_village, sample_ID, bag_ID_tap, bag_ID_stored, cf_pa_binary, sample_type, ec_pa_binary, cf_log, ec_log)%>%
  mutate(data_round = "R1")%>%
  mutate(pooled_round = "FU")

idexx_r2_tab <- idexx_r2%>%
  dplyr::select(assignment, village, block, panchayat_village, sample_ID, bag_ID_tap, bag_ID_stored, cf_pa_binary, sample_type, ec_pa_binary, cf_log, ec_log)%>%
  mutate(data_round = "R2")%>%
  mutate(pooled_round = "FU")

idexx_r3_tab <- idexx_r3%>%
  dplyr::select(assignment, village, block, panchayat_village, sample_ID, bag_ID_tap, bag_ID_stored, cf_pa_binary, sample_type, ec_pa_binary, cf_log, ec_log)%>%
  mutate(data_round = "R3")%>%
  mutate(pooled_round = "FU")

#combining
idexx_tab <- rbind(idexx_tab, idexx_r1_tab, idexx_r2_tab, idexx_r3_tab)
...
x <- idexx_ids%>%
  count(sample_ID, data_round)%>% 
  filter(n > 1)
#x_BL <- idexx%>%
 # filter(sample_ID %in% x$sample_ID)
#7 duplicate IDs that need to be handled
...
#Stored Water
all_models <- lapply(c("cf_pa_stored", "ec_pa_stored"),
                 function(var) {
                   formula <- as.formula(paste(var, "~ assignment + block + panchayat_village"))
                   model <- glm(formula, data = idexx_tab_stored, family = binomial)
                   # Adjust for clustering at the village level
                   cluster_vcov <- vcovCL(model, cluster = ~village)
                   clustered_se <- sqrt(diag(cluster_vcov))
                   # Get the coefficients with clustered standard errors
                   clustered_results <- coeftest(model, vcov. = cluster_vcov)
                   #Tidying models -- need to understand how to use clustered results option
                   list(model = model,
                        table = tbl_regression(model, exponentiate = TRUE)) #Tidying models
                   })
#Assigning names to models
names(all_models) <- c("prim_source_jjm", "sec_source", "jjm_drinking", "water_treat_binary")
...
#Naming gt tables
names(gt_tables) <- c("prim_source_jjm", "sec_source", "jjm_drinking", "water_treat_binary")
...
#Running regression models for each variable in the dataset
all_models <- lapply(c("prim_source_jjm", "sec_source", "jjm_drinking", "water_treat_binary"),
                 function(var) {
                   formula <- as.formula(paste(var, "~ assignment + block + panchayat_village"))
                   model <- glm(formula, data = el_tab, family = binomial)
                   # Adjust for clustering at the village level
                   cluster_vcov <- vcovCL(model, cluster = ~village)
                   clustered_se <- sqrt(diag(cluster_vcov))
                   # Get the coefficients with clustered standard errors
...
## Old robust standard error code
