...
# cen <- cen%>%
#   mutate(hhhead_gender = ifelse(hhhead_gender == "Female", 1, 0))%>%
#   mutate(read_write_1 = ifelse(read_write_1 == "Yes", 1, 0))

# Only used when needed to output
...
```{r New Baseline Desc Stats Code}
...
outcome_vars <- c("ec_log") #Selecting outcome variables
...
cen_vars <- c("unique_id","prim_source_jjm", "sec_source",
                  "jjm_drinking", "water_treat_binary" 
...
bl_vars <- c("unique_id","treat_time_5min") #But can I use this to control for baseline data if the dataset is partial?
#Selecting census variables to keep
bl_model <- bl%>%
...
idexx_bl_tap <- idexx%>%
  filter(sample_type == "Tap")%>%
  dplyr::select(unique_id, cf_pa_binary, ec_pa_binary#, cf_log, ec_log
                )%>%
  rename(bl_cf_pa_tap = "cf_pa_binary")%>%
  rename(bl_ec_pa_tap = "ec_pa_binary")%>%
  # rename(cf_log_tap = "cf_log")%>%
  # rename(ec_log_tap = "ec_log")%>%
...
idexx_bl_stored <- idexx%>%
  ...
  rename(bl_cf_pa_stored = "cf_pa_binary")%>%
  rename(bl_ec_pa_stored = "ec_pa_binary")%>%
  # rename(cf_log_stored = "cf_log")%>%
  # rename(ec_log_stored = "ec_log")%>%
  mutate(unique_id = as.character(unique_id))
...
idexx_bl_abr_tap <- idexx_abr%>%
  filter(data_round == "BL")%>%
  filter(sample_type == "Tap")%>%
  dplyr::select(unique_id, ec_pa_binary
                )%>%
  rename(bl_ec_abr_tap = "ec_pa_binary")%>%
  mutate(unique_id = as.character(unique_id))
...
idexx_bl_abr_stored <- idexx_abr%>%
  ...
  rename(bl_ec_abr_stored = "ec_pa_binary")%>%
  mutate(unique_id = as.character(unique_id))
...
el_model <- el%>%
  left_join(cen_model, by = "unique_id")%>%
  left_join(bl_model, by = "unique_id")
...
idexx_fu_tap <- idexx_fu_tap%>%
  left_join(idexx_bl_tap, by = "unique_id")

idexx_fu_stored <- idexx_fu_stored%>%
  left_join(idexx_bl_stored, by = "unique_id")
...
idexx_abr_tap <- idexx_abr_tap%>%
  left_join(idexx_bl_abr_tap, by = "unique_id")

idexx_abr_stored <- idexx_abr_stored%>%
  left_join(idexx_bl_abr_stored, by = "unique_id")
