...
#bl <- read_stata(paste0(user_path(), "/3_final/1_1_Baseline_Census_HH_clean_consented.dta")) #NB: repalcing with the latest version

#NB: The file below is the merged version of bl census and bl hh survey; this has been archived now (3_final/Z_Archive/); alternatively "/8_Intermediate Datasets/1_2_Followup.dta" can be sued here generated from 1_2_A_Followup_cleaning.do
#Baseline Census
...
#el <- read_stata(paste0(user_path(),"3_final/1_10_Endline_Census_HH_clean_consented.dta" )) #NB: replacing with the latest dataset 
...
#Median Stored water time
# x <- all_rounds%>%
#   filter(is.na(stored_water_time) == FALSE)
# 
# median(x$stored_water_time)
...
cl_p4 <- ggplot(data = mon_summary_percent_2) + 
  ...
  labs(x = "Sample Location", 
       y = "% Positive for Free Chlorine (> 0.1 mg/L)",
       title = "Positive chlorine samples during weekly monitoring - February-September 2024"
       )+
  theme_bw() + 
  theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1))
...
#Making crosstab of the same chlorine vs idexx results

#Renaming variables for readability
idexx_crosstab <- idexx_crosstab%>%
  rename(`E. coli Presence/Absence` = "ec_pa")%>%
  rename(`Total Coliform Presence/Absence` = "cf_pa")%>%
  rename(`Tap Free Chlorine Concentration` = "fc_tap_pa_1")%>%
  rename(`Stored Free Chlorine Concentration` = "fc_stored_pa_1")

p_crosstab_tap_ec <- idexx_crosstab%>%
  crosstable( `Tap Free Chlorine Concentration`, by = `E. coli Presence/Absence`, total = "both")%>%
  as_flextable(keep_id = FALSE)

p_crosstab_stored_ec <- idexx_crosstab%>%
  crosstable(`Stored Free Chlorine Concentration` , by = `E. coli Presence/Absence`, total = "both")%>%
  as_flextable(keep_id = FALSE)

p_crosstab_tap_cf <- idexx_crosstab%>%
  crosstable( `Tap Free Chlorine Concentration`, by = `Total Coliform Presence/Absence`, total = "both")%>%
  as_flextable(keep_id = FALSE)

p_crosstab_stored_cf <- idexx_crosstab%>%
  crosstable(`Stored Free Chlorine Concentration` , by = `Total Coliform Presence/Absence`, total = "both")%>%
  as_flextable(keep_id = FALSE)
...
```{r Regression Results -- Pooled Household Surveys, results='asis', output.dir = "C:/Users/jerem/Dropbox/Apps/Overleaf/India ILC Pilot- analysis"}
...
fu_rounds <- all_rounds%>%
  filter(data_round != "BL")%>%
  filter(available_jjm == "Both stored and running water are from JJM")
...
#Running regression models -- linear models based on Akito's code

#Variables
outcome_vars <- c("fc_tap_avg", "tc_tap_avg", "fc_stored_avg", "tc_stored_avg")

#Running all models
#Logistic regression
# all_models <- lapply(outcome_vars, 
#                      function(var){ #This functions "subsets" the ilc_glm function
#                        ilc_glm(data = fu_rounds, var)
#                      }
#                        )
