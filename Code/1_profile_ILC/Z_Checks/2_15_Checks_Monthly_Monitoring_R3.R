#------------------------------------------------ 
# title: "Code for Checks for High-frequency Follow Ups for R3 monsoon IDEXX"
...
#Monthly IDEXX data
idexx <- read_csv(paste0(user_path(),"/3_final/idexx_monthly_master_cleaned_R3.csv"))

#Monthly household survey data
ms <- read_csv(paste0(user_path(), "/3_final/1_10_monthly_follow_up_cleaned_R3.csv"))
...
stargazer(df.progress, summary=F, title= "Overall Progress: Monthly IDEXX Survey",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Progress_idexx_R3.tex"))
...
starpolishr::star_tex_write(star.out, file = paste0(overleaf(), "Table/Table_Progress_idexx_R3.tex"))
...
stargazer(df.rep, summary=F, title= "Distribution of Replacements by Village",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/replacements_idexx_R3.tex"))
...
stargazer(replace_percentage, summary=F, title= "Reasons of Replacement breakdown",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/replacement_reason_idexx_R3.tex"))
...
stargazer(summary_counts, summary=F, title= "Cases where TC is lower than FC",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/TC_lower_than_FC_idexx_R3.tex"))
...
# Save the plot
ggplot2::ggsave(paste0(overleaf(), "Figure/boxplot_test_types_R3.png"), tests, bg = "white", width = 5, height = 5, dpi = 200)
...
for (var in names(plots)) {
  ggsave(paste0(overleaf(),"Figure/scatter_plot_R3", var, ".png"), plot = plots[[var]], bg = "white", width = 10, height = 6)
}
...
ggplot2::ggsave(paste0(overleaf(), "Figure/boxplot_village_test_types_R3.png"), tests, bg = "white", width = 5, height = 5, dpi = 200)
...
stargazer(water_source_percentage, summary=F, title= "Primary water source breakdown",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Prim_source_idexx_R3.tex"))
...
stargazer(response_percentages, summary=F, title= "",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/sec_source_idexx_R3.tex"))
...
stargazer(sums_long, 
          ...
          float = FALSE, 
          rownames = FALSE, 
          out = paste0(overleaf(), "Table/Sec_source_break_idexx_R3.tex"))
...
stargazer(combined_table, 
          ...
          float = FALSE, 
          rownames = FALSE, 
          out = paste0(overleaf(), "Table/Sec_source_combined_breakdown_R3.tex"))
...
starpolishr::star_tex_write(star.out,  file =paste0(overleaf(),"Table/Sec_source_combined_breakdown_R3.tex"))
...
chlorine_stats_wide <- stargazer(chlorine_stats_wide, summary=FALSE,
                                 ...
                                 out=paste0(overleaf(),"Table/chlorine_stats_monthly_r3.tex"))
