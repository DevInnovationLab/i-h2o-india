...
resp_available <- df.temp %>%  filter(R_FU1_resp_available == 1 & R_FU1_replacement == 0 ) %>% 
  select(R_FU1_r_cen_village_name_str) %>%
  group_by(R_FU1_r_cen_village_name_str) %>% mutate("OG Found" =n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU1_r_cen_village_name_str)
resp_notavailable <- df.temp %>%  filter(R_FU1_resp_available != 1  ) %>% 
  select(R_FU1_r_cen_village_name_str) %>%
  group_by(R_FU1_r_cen_village_name_str) %>% mutate("NOT Found" =n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU1_r_cen_village_name_str)

consented <- df.temp  %>% filter(R_FU1_consent == 1) %>% select(R_FU1_r_cen_village_name_str) %>%
  group_by(R_FU1_r_cen_village_name_str) %>% mutate(Consented=n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU1_r_cen_village_name_str)

refused <- df.temp %>% filter(R_FU1_consent != 1) %>% select(R_FU1_r_cen_village_name_str) %>%
  group_by(R_FU1_r_cen_village_name_str) %>% mutate(Refused=n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU1_r_cen_village_name_str)

replacements <- df.temp %>%  filter( R_FU1_replacement == 1 ) %>% 
  select(R_FU1_r_cen_village_name_str) %>%
  group_by(R_FU1_r_cen_village_name_str) %>% mutate(Replacements=n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU1_r_cen_village_name_str)
...
df.progress <- left_join(submissions,resp_available)%>% left_join(resp_notavailable) %>% left_join(replacements) %>% 
  ...
  rename(Village = R_FU1_r_cen_village_name_str)
...
stargazer(df.progress, summary=F, title= "Overall Progress: Follow Up Round 1 HH Survey",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Progress_HH_Survey_R1.tex"))
...
tab[is.na(tab)] <- 0 
tab <- tab %>% rename(Village = R_FU1_r_cen_village_name_str)
print(tab)

stargazer(tab,out= paste0(overleaf(), "Table/Table_survey_by_date_village_R1.tex"), summary=F, float=F,rownames = F,
...
tab[is.na(tab)] <- 0 
tab <- tab %>% rename(Village = R_FU1_r_cen_village_name_str, Enumerator =R_FU1_enum_name_label  )
print(tab)

stargazer(tab,out= paste0(overleaf(), "Table/Table_survey_by_enum_date_village_R1.tex"), summary=F, float=F,rownames = F,
          covariate.labels=NULL)
...
df.duration.enum <- df.duration.enum %>% rename(Enumerator = R_FU1_enum_name_label )
star.out <- stargazer(df.duration.enum, summary=F, title= "Duraction by section Follow Up R1 HH Survey by enumerator",float=F,rownames = F,covariate.labels=NULL)
star.out <- sub("cccccccc", "lccccccc", star.out)
starpolishr::star_tex_write(star.out,  file =paste0(overleaf(),"Table/Table_Duration_enum_HH_Survey_R1.tex"))
...
starpolishr::star_tex_write(star.out,  file =paste0(overleaf(),"Table/Table_Duration_WQ_enum_HH_Survey_R1.tex"))
...
stargazer(df.dk.enum, summary=F, title= "Count of DK by enumerator",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_DK_enum_HH_Survey_R1.tex"))
...
df.888.enum <- df.temp.consent %>%
  ...
  transmute(R_FU1_enum_name_label, sum_dk = rowSums(.[-1], na.rm = T)) %>% 
  rename(Enumerator = R_FU1_enum_name_label, "Count of 888" = sum_dk)

stargazer(df.888.enum, summary=F, title= "Count of DK by enumerator",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_888_enum_HH_Survey_R1.tex"))
...
chlorine <- bl%>%
  pivot_longer(cols = c(R_FU1_fc_tap,R_FU1_wq_tap_fc_again , R_FU1_tc_tap,R_FU1_wq_tap_tc_again, R_FU1_fc_stored, R_FU1_wq_chlorine_storedfc_again, R_FU1_tc_stored, R_FU1_wq_chlorine_storedtc_again), 
               values_to = "chlorine_concentration", names_to = "chlorine_test_type") %>% 
  mutate(chlorine_test_type = ifelse(chlorine_test_type =="R_FU1_fc_tap", "Running FC", 
                              ifelse(chlorine_test_type == "R_FU1_wq_tap_fc_again", "Running FC- Again",
                              ifelse(chlorine_test_type =="R_FU1_tc_tap", "Running TC", 
                              ifelse(chlorine_test_type == "R_FU1_wq_tap_tc_again", "Running TC- Again", 
                              ifelse(chlorine_test_type == "R_FU1_fc_stored", "Stored FC",
                              ifelse(chlorine_test_type == "R_FU1_wq_chlorine_storedfc_again", "Stored FC- Again", 
                              ifelse(chlorine_test_type == "R_FU1_tc_stored", "Stored TC",
                              ifelse(chlorine_test_type == "R_FU1_wq_chlorine_storedtc_again", "Stored TC- Again" , "Missing")))))))))
...
chlorine_check <- chlorine%>%
  ...
  dplyr::select(R_FU1_enum_name_label,R_FU1_r_cen_village_name_str) %>% 
...
stargazer(chlorine_check, summary=F, title= "Count of Chlorine reading greater than 0.10",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Chlorine_high_HH_Survey_R1.tex"))
...
df.error.enum <- chlorine %>%   filter(chlorine_concentration == 999) %>% 
  dplyr::select(R_FU1_enum_name_label, R_FU1_r_cen_village_name_str, R_FU1_colorimeter_id, R_FU1_colorimeter_type,chlorine_test_type) %>% 
  group_by(R_FU1_enum_name_label,chlorine_test_type) %>%
  mutate(sum_dk = n()) %>% 
  rename(Enumerator = R_FU1_enum_name_label, Count_of_Errors =sum_dk, Village = R_FU1_r_cen_village_name_str, Colorimeter_ID = R_FU1_colorimeter_id, Colorimeter_Type = R_FU1_colorimeter_type) %>% 
  unique()
  
 

stargazer(df.error.enum, summary=F, title= "Count of Errors by enumerator",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_count_error_enum_HH_Survey_R1.tex"))
...
#Arranging data so chlorine test data is in one column
df.sample <- bl%>%
  pivot_longer(cols = c("R_FU1_wq_stored_bag", "R_FU1_wq_running_bag"), values_to = "yes_no", names_to = "sample_type")

#Removing NAs from no respondent being available
df.sample <- df.sample%>%
  filter(is.na(yes_no) == FALSE) %>% dplyr::select(R_FU1_enum_name_label, R_FU1_r_cen_village_name_str, sample_type, yes_no)
#Frequency of sample collection by enumerator
df.sample.stored.enum <- df.sample%>% filter(sample_type == "R_FU1_wq_stored_bag") %>% 
  group_by(R_FU1_enum_name_label, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_FU1_enum_name_label, Count_of_Sample) %>% 
  rename(Enumerator = R_FU1_enum_name_label, "Count-Stored Water" = Count_of_Sample ) %>% 
  unique() 

df.sample.running.enum <- df.sample%>% filter(sample_type == "R_FU1_wq_running_bag") %>% 
  group_by(R_FU1_enum_name_label, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>%
  dplyr::select(R_FU1_enum_name_label, Count_of_Sample) %>% 
  rename(Enumerator = R_FU1_enum_name_label, "Count-Running Water" = Count_of_Sample ) %>% 
  unique()
...
stargazer(df.sample.enum, summary=F, title= "Count of Sample collection by enum",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_sample_enum_HH_Survey_R1.tex"))


df.chlorine <- bl%>%
  pivot_longer(cols = c("R_FU1_fc_stored", "R_FU1_fc_tap", "R_FU1_tc_stored", "R_FU1_tc_tap"), values_to = "yes_no", names_to = "sample_type")
#Frequency of sample collection by enumerator
df.chlorine.vil.fc.stored <- df.chlorine%>% filter(sample_type == "R_FU1_fc_stored") %>% 
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str, "Count-FCl Stored" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.tc.stored <- df.chlorine%>% filter(sample_type == "R_FU1_tc_stored") %>% 
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str, "Count-TCl Stored" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.fc.tap <- df.chlorine%>% filter(sample_type == "R_FU1_fc_tap") %>% 
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str, "Count-FCl Running" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.tc.tap <- df.chlorine%>% filter(sample_type == "R_FU1_tc_tap") %>%
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str, "Count-TCl Running" = Count_of_Sample ) %>% 
  unique() 
...
stargazer(df.chlorine.vil, summary=F, title= "Count of  Chlorine Tests by village",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_chlorine_village_HH_Survey_R1.tex"))
...
consent_dur <- df.temp.consent %>%  filter(FU1_consent_dur_min > 0) %>%
  dplyr:: summarise(mean = round(mean(FU1_consent_dur_min),1), min = round(min(FU1_consent_dur_min),1), max = round(max(FU1_consent_dur_min),1) ) %>%
  mutate(variable = "Consent")
secA_dur <- df.temp.consent %>% filter(FU1_secA_dur_min > 0) %>%
  dplyr:: summarise(mean = round(mean(FU1_secA_dur_min),1), min = round(min(FU1_secA_dur_min),1), max = round(max(FU1_secA_dur_min),1) ) %>%
  mutate(variable = "Sec A: WASH Access")
secB_dur <- df.temp.consent %>% filter(FU1_secB_dur_min > 0) %>%
  dplyr:: summarise(mean = round(mean(FU1_secB_dur_min),1), min = round(min(FU1_secB_dur_min),1), max = round(max(FU1_secB_dur_min),1) ) %>%
  mutate(variable = "Sec B: Government Tap")
secC_dur <- df.temp.consent %>% filter(FU1_secC_dur_min > 0) %>%
  dplyr:: summarise(mean = round(mean(FU1_secC_dur_min),1), min = round(min(FU1_secC_dur_min),1), max = round(max(FU1_secC_dur_min),1) ) %>%
  mutate(variable = "Sec C: Chlorination Perceptions")
secE_dur <- df.temp.consent %>% filter(FU1_secE_dur_min > 0) %>%
...
stargazer(df.duration, summary=F, title= "Duraction by section Baseline HH Survey",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Duration_HH_Survey_R1.tex"))
...
secE_dur_chlorine <- secE_dur %>% filter(FU1_test_type == 1) %>% 
  mutate(FU1_secE_dur_min  = ifelse(FU1_secE_dur_min<0 , NA, FU1_secE_dur_min)) %>% 
  dplyr:: summarise(mean = round(mean(FU1_secE_dur_min, na.rm = T),1), min = round(min(FU1_secE_dur_min,  na.rm = T),1), max = round(max(FU1_secE_dur_min,  na.rm = T),1) ) %>%
  mutate(variable = "Chlorine Testing")
secE_dur_both <- df.temp.consent %>% filter(FU1_test_type == 2) %>% 
  mutate(FU1_secE_dur_min  = ifelse(FU1_secE_dur_min<0 , NA, FU1_secE_dur_min)) %>% 
  dplyr:: summarise(mean = round(mean(FU1_secE_dur_min, na.rm = T),1), min = round(min(FU1_secE_dur_min,  na.rm = T),1), max = round(max(FU1_secE_dur_min,  na.rm = T),1) ) %>%
...
stargazer(df.duration.wq, summary=F, title= "Duraction for Water Quality Test -  Baseline HH Survey",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Duration_WQ_HH_Survey_R1.tex"))
...
df.count.stored <- df.sample%>% filter(sample_type == "R_FU1_wq_stored_bag") %>%
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str,  "Count-Stored Water" = Count_of_Sample ) %>% 
  unique()
df.count.running <- df.sample%>% filter(sample_type == "R_FU1_wq_running_bag") %>%
  group_by(R_FU1_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_FU1_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_FU1_r_cen_village_name_str, "Count-Running Water" = Count_of_Sample ) %>% 
  unique()
...
stargazer(df.count, summary=F, title= "Count of Sample collection",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_sample_HH_Survey_R1.tex"))
...
stargazer(df.count, summary=F, title= "Count of Testing not possible",float=F,rownames = F,
          covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_test_not_possible_HH_Survey_R1.tex"))
...
df.consistency <- df.temp.consent %>% filter(R_FU1_tap_use_drinking_yesno == 0 ) %>%
  ...
  mutate(R_FU1_tap_use_drinking = ifelse(R_FU1_tap_use_drinking == 1, "Today", 
                                  ...
                                  ifelse(R_FU1_tap_use_drinking == 3, "Earlier this week", 
                                  ifelse(R_FU1_tap_use_drinking == 4, "Earlier this month", 
                                  ifelse(R_FU1_tap_use_drinking == 5,"Not used for drinking", 
                                  ifelse(R_FU1_tap_use_drinking == -77,"Other", NA))))))) %>%
