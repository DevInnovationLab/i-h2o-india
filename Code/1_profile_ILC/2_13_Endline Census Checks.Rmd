...
#Summarizing a few key village characteristics
...
tab <- df.temp.consent %>% filter(date >= as.Date("2024-04-15")) %>%
  dplyr::select(R_E_enum_name_label,R_E_r_cen_village_name_str, R_E_cen_num_hhmembers, R_E_cen_num_childbelow5, R_E_visit_num) %>%
  rename(Visit_Number = R_E_visit_num,
         Number_of_children = R_E_cen_num_childbelow5,
         Number_of_HH_members = R_E_cen_num_hhmembers)

table1(data = tab, ~ Visit_Number + Number_of_children + Number_of_HH_members)
...
tab <- df.temp.consent%>%
  dplyr::select(R_E_r_cen_village_name_str, R_E_water_source_prim, R_E_sec_source_reason_999,
                R_E_sec_source_reason__77, R_E_water_sec_yn, R_E_water_sec_freq)%>%
  group_by(R_E_r_cen_village_name_str) %>% 
  rename(Village = R_E_r_cen_village_name_str)

tab$R_E_water_source_prim <- to_label(tab$R_E_water_source_prim)
tab$R_E_water_sec_yn <- to_label(tab$R_E_water_sec_yn)
tab$R_E_water_sec_freq <- to_label(tab$R_E_water_sec_freq)
...
#Table1 Output
#table1(data = tab, ~ R_E_water_source_prim  | Village)
...
#
tab_2 <-tab%>%
  dplyr::select(Village, R_E_water_sec_yn, R_E_water_sec_freq, R_E_sec_source_reason_999, R_E_sec_source_reason__77) 

#Table1 output
table1(data = tab_2, ~ R_E_water_sec_yn + R_E_water_sec_freq + R_E_sec_source_reason_999 + R_E_sec_source_reason__77)
