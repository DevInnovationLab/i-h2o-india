...
```{r enumerator productivity tables, eval=FALSE}

#------------------------ Distribution of surveys by dates & villages ----------------------------------------#
date.plt<- df.temp %>% filter(R_E_consent == 1) %>% filter(date >= as.Date("2024-02-15")) %>%
...
total_dur_enum <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% 
  group_by(R_E_enum_name_label) %>%
  dplyr:: summarise("Overall" = round(mean(R_E_duration_end/60),1))  %>% ungroup()
...
# R_E_water_qual_test == 1 but whether only chlorine testing or both chlorine and sample collection dependent on R_E_ecoli_yn and R_E_available_jjm

df.temp.consent <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% mutate(FU3_test_type = ifelse(R_E_water_qual_test == 1 & R_E_ecoli_yn == 0, 1, 
                                                                                                               ifelse(R_E_water_qual_test == 1 & R_E_ecoli_yn == 1 &
                                                                                                                        (R_E_available_jjm == 1 | R_E_available_jjm == 2),2,
                                                                                                                      ifelse(R_E_water_qual_test == 1 & R_E_ecoli_yn == 1 & (R_E_available_jjm == 3 | R_E_available_jjm == 4), 1, 0))))
...
df.duration.wq.enum <- df.duration.wq.enum %>% rename(Enumerator = R_E_enum_name_label )
...
df.dk.enum <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>%
  group_by(R_E_enum_name_label) %>% dplyr::select(-R_E_fc_stored,-R_E_tc_stored, -R_E_wq_chlorine_storedfc_again, -R_E_wq_chlorine_storedtc_again,
                                             -R_E_fc_tap, -R_E_tc_tap,-R_E_wq_tap_fc_again,-R_E_wq_tap_tc_again, -R_E_r_cen_a39_phone_name_1, - R_E_r_cen_a39_phone_name_2) %>%
...
chlorine <- bl%>%
  pivot_longer(cols = c(R_E_fc_tap,R_E_wq_tap_fc_again , R_E_tc_tap,R_E_wq_tap_tc_again, R_E_fc_stored, R_E_wq_chlorine_storedfc_again, R_E_tc_stored, R_E_wq_chlorine_storedtc_again), 
               values_to = "chlorine_concentration", names_to = "chlorine_test_type") %>% 
  mutate(chlorine_test_type = ifelse(chlorine_test_type =="R_E_fc_tap", "Running FC", 
                                     ifelse(chlorine_test_type == "R_E_wq_tap_fc_again", "Running FC- Again",
                                            ifelse(chlorine_test_type =="R_E_tc_tap", "Running TC", 
                                                   ifelse(chlorine_test_type == "R_E_wq_tap_tc_again", "Running TC- Again", 
                                                          ifelse(chlorine_test_type == "R_E_fc_stored", "Stored FC",
                                                                 ifelse(chlorine_test_type == "R_E_wq_chlorine_storedfc_again", "Stored FC- Again", 
                                                                        ifelse(chlorine_test_type == "R_E_tc_stored", "Stored TC",
                                                                               ifelse(chlorine_test_type == "R_E_wq_chlorine_storedtc_again", "Stored TC- Again" , "Missing")))))))))
...
chlorine_check <- chlorine%>%
  ...
  dplyr::select(R_E_enum_name_label,R_E_r_cen_village_name_str) %>% 
...
df.error.enum <- chlorine %>%   filter(chlorine_concentration == 999) %>% 
  dplyr::select(R_E_enum_name_label, R_E_r_cen_village_name_str, R_E_colorimeter_id, R_E_colorimeter_type,chlorine_test_type) %>% 
  group_by(R_E_enum_name_label,chlorine_test_type) %>%
  mutate(sum_dk = n()) %>% 
  rename(Enumerator = R_E_enum_name_label, Count_of_Errors =sum_dk, Village = R_E_r_cen_village_name_str, Colorimeter_ID = R_E_colorimeter_id, Colorimeter_Type = R_E_colorimeter_type) %>% 
  unique()
...
#Arranging data so chlorine test data is in one column
df.sample <- bl%>%
  pivot_longer(cols = c("R_E_wq_stored_bag", "R_E_wq_running_bag"), values_to = "yes_no", names_to = "sample_type")

#Removing NAs from no respondent being available
df.sample <- df.sample%>%
  filter(is.na(yes_no) == FALSE) %>% dplyr::select(R_E_enum_name_label, R_E_r_cen_village_name_str, sample_type, yes_no)
#Frequency of sample collection by enumerator
df.sample.stored.enum <- df.sample%>% filter(sample_type == "R_E_wq_stored_bag") %>% 
  group_by(R_E_enum_name_label, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_E_enum_name_label, Count_of_Sample) %>% 
  rename(Enumerator = R_E_enum_name_label, "Count-Stored Water" = Count_of_Sample ) %>% 
  unique() 

df.sample.running.enum <- df.sample%>% filter(sample_type == "R_E_wq_running_bag") %>% 
  group_by(R_E_enum_name_label, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>%
  dplyr::select(R_E_enum_name_label, Count_of_Sample) %>% 
  rename(Enumerator = R_E_enum_name_label, "Count-Running Water" = Count_of_Sample ) %>% 
  unique()
...
df.chlorine <- bl%>%
  pivot_longer(cols = c("R_E_fc_stored", "R_E_fc_tap", "R_E_tc_stored", "R_E_tc_tap"), values_to = "yes_no", names_to = "sample_type")
#Frequency of sample collection by enumerator
df.chlorine.vil.fc.stored <- df.chlorine%>% filter(sample_type == "R_E_fc_stored") %>% 
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str, "Count-FCl Stored" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.tc.stored <- df.chlorine%>% filter(sample_type == "R_E_tc_stored") %>% 
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str, "Count-TCl Stored" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.fc.tap <- df.chlorine%>% filter(sample_type == "R_E_fc_tap") %>% 
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str, "Count-FCl Running" = Count_of_Sample ) %>% 
  unique() 

df.chlorine.vil.tc.tap <- df.chlorine%>% filter(sample_type == "R_E_tc_tap") %>%
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = sum(!is.na(yes_no))) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str, "Count-TCl Running" = Count_of_Sample ) %>% 
  unique() 
...
```{r overall variable checks, eval = FALSE}
#------------------------  Overall checks ------------------------  
...
overall_mean <- df.temp.consent %>% filter(date >= as.Date("2024-02-12")) %>% filter(R_E_duration_end > 0) %>%
  dplyr:: summarise(mean = round(mean(R_E_duration_end/60),1), min = round(min(R_E_duration_end/60),1), max = round(max(R_E_duration_end/60),1) ) %>%
...
secE_dur <- df.temp.consent %>% filter(date >= as.Date("2024-02-12")) %>% mutate(FU3_test_type = ifelse(R_E_water_qual_test == 1 & R_E_ecoli_yn == 0 &
                                                                                                          (R_E_available_jjm == 3 | R_E_available_jjm == 4), 1, 
                                                                                                        ifelse(R_E_water_qual_test == 1 & R_E_ecoli_yn == 1 &
                                                                                                                 (R_E_available_jjm == 1 | R_E_available_jjm == 2),2, 0)))
...
df.count.stored <- df.sample%>% filter(sample_type == "R_E_wq_stored_bag") %>%
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str,  "Count-Stored Water" = Count_of_Sample ) %>% 
  unique()
df.count.running <- df.sample%>% filter(sample_type == "R_E_wq_running_bag") %>%
  group_by(R_E_r_cen_village_name_str, sample_type) %>% 
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
  dplyr::select(R_E_r_cen_village_name_str, Count_of_Sample) %>% 
  rename(Village = R_E_r_cen_village_name_str, "Count-Running Water" = Count_of_Sample ) %>% 
  unique()
...
df.sample.collection <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% filter(R_E_ecoli_yn == 1 & (R_E_available_jjm ==3 | R_E_available_jjm == 4)) %>%
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
...
df.water.testing <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% filter( R_E_water_qual_test == 0) %>%
  mutate(Count_of_Sample = n()) %>% ungroup()  %>% 
...
df.consistency <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% filter(R_E_tap_use_drinking_yesno == 0 ) %>%
  dplyr::select(R_E_r_cen_village_name_str, R_E_enum_name_label, R_E_tap_use_drinking , unique_id_num ) %>% 
...
df.consistency_the_other_way <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>% filter(R_E_tap_use_drinking_yesno == 1 & R_E_tap_use_drinking == 5) %>%
...
df.shadow <- df.temp.consent %>% filter(date >= as.Date("2024-02-15")) %>%   dplyr::select(R_E_enum_name_label,R_E_r_cen_village_name_str, R_E_survey_accompany_num) %>%
  group_by(R_E_enum_name_label,R_E_r_cen_village_name_str ) %>% 
  dplyr:: summarise(mean = round(mean(R_E_survey_accompany_num),1), 
                    min = round(min(R_E_survey_accompany_num),1), max = round(max(R_E_survey_accompany_num),1)) %>%
  rename(Village = R_E_r_cen_village_name_str, Enumerator = R_E_enum_name_label) 
