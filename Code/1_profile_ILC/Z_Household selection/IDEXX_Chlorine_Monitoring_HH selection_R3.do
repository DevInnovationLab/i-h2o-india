...
use "${DataFinal}1_1_Census_cleaned_consented - 14th Aug.dta", clear
...
***********************************************************************
	...
	* Sort among primary household
	...
	save "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_`value'_IDEXX_5 Sep 2024.dta", replace
...
use "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_`value'_IDEXX_5 Sep 2024.dta", clear
*append using "${DataPre}Selected_30501_20 Oct 2023.dta"
*append using "${DataPre}Selected_30202_20 Oct 2023.dta"
save "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_HHs_IDEXX_chlorine_testing_R3.dta", replace


use   "${DataFinal}1_1_Census_cleaned_consented - 14th Aug.dta", clear
* Merge_WS==1 means they do not drink water from the JJM tap
merge 1:1 unique_id using "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_HHs_IDEXX_chlorine_testing_R3.dta", keep(master matched) gen(Merge_WS)
...
export excel ID R_Cen_block_name R_Cen_village_name_str R_Cen_hamlet_name R_Cen_saahi_name R_Cen_landmark R_Cen_time_availability Enumerator_Assigned S_BLWQ  using "${pilot}IDEXX_Chlorine_Monitoring_Monthly_Tracker/Supervisor_IDEXX_Tracker_`vill'_5 Sep 2024.xlsx" if S_BLS==1, sheet("Sheet1", replace) firstrow(varlabels) cell(A1) 

sort R_Cen_village_name_str S_BLWQ
export excel ID R_Cen_block_name R_Cen_village_name_str R_Cen_hamlet_name R_Cen_saahi_name R_Cen_landmark R_Cen_time_availability Enumerator_Assigned S_BLWQ  using "${pilot}IDEXX_Chlorine_Monitoring_Monthly_Tracker/Supervisor_IDEXX_Tracker_`vill'_5 Sep 2024_Replacement list.xlsx" if S_BLS==2, sheet("Sheet1", replace) firstrow(varlabels) cell(A1) 
...
use "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_10101_IDEXX_5 Sep 2024.dta", clear

foreach i of local mylist {
	
append using "${DataPre}IDEXX_Chlorine_Monitoring_Monthly_Preload\Selected_`i'_IDEXX_5 Sep 2024.dta"
}

save "${DataTemp}Appended-IDEXX_village_HH_R3.dta", replace

use   "${DataFinal}1_1_Census_cleaned_consented - 14th Aug.dta", clear
merge 1:1 unique_id using "${DataTemp}Appended-IDEXX_village_HH_R3.dta"
