#------------------------------------------------ 
# title: "Code for Checks for High-frequency Follow Ups for Monsoon R3 of idexx"
# author: "Nihairka, Jeremy Lowe, Archi Gupta"
...
user_path <- function() {
  ...
  else if (user=="uchicago"){  #Niharika's filepath
...
#Monthly Surveys - ms
ms <- read_csv(paste0(user_path(), "1_raw/Chlorine and IDEXX Monitoring Survey R3_WIDE.csv"))
#View(ms)
...
#IDEXX results
idexx <- read_csv(paste0(user_path(), "1_raw/IDEXX Results Reporting - Oct 2024_WIDE.csv"))
...
# Manually correcting stray values - data entry error: coded an ABR sample as non-ABR sample or Lab blank
idexx <- idexx %>%
  mutate(ABR = case_when(
    ...
    end_comments == "LAB BLANK" & endtime.x =="22-Oct-2024 08:55:51" & sample_ID == "10581" & unique_bag_id == "91160" ~ 1, # Case 3
...
write_csv(idexx,paste0(user_path(),"/5_lab data/idexx/cleaned/idexx_monthly_master_cleaned_R3.csv"))
write_csv(idexx,paste0(user_path(),"/3_final/idexx_monthly_master_cleaned_R3.csv"))


#Monthly Survey
write_csv(ms, paste0(user_path(), "/3_final/1_10_monthly_follow_up_cleaned_R3.csv"))
