...
chlorine <- chlorine %>% filter(Date >= as.Date("2024-02-01"))
...
for (i in village_list){
  df.vil.cl <-   df.tap %>% filter(village == i) %>% filter(chlorine_test_type != "nearest_tap_tc" &  chlorine_test_type != "farthest_tap_tc"  )    
  p <- ggplot(df.vil.cl, aes(x = Date, y = chlorine_concentration, color=Distance)) +
  geom_line(linewidth = 1.00)  + geom_point() + 
  labs(title = " Concentration of Chlorine",
       x = "Date",
  ...
  theme(
    ...
    geom_hline(yintercept=0.6, linetype="dotdash", color = "blue", linewidth=0.5) +   
...
setwd(paste0(overleaf(), "/Figure"))
for (i in village_list) {
    file_name = paste("Village_tap_for_GV_", i, ".png", sep="")
    png(file_name)
