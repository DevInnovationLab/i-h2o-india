...
use "${DataRaw}1_1_Census_new.dta", clear
...
gen submissiondate = substr(R_Cen_submissiondate, 1, 9)
gen starttime = substr(R_Cen_starttime, 1, 9)
gen endtime = substr(R_Cen_endtime, 1, 9)
...
*ssc install fre
fre submissiondate
*gen T = string(submissiondate, "%td") 
*labmask submissiondate, values(T) 
...
qui bys submissiondate: gen daily_total = _N
...
preserve
collapse daily_total daily_avg, by (submissiondate)
...
sort R_Cen_village_str  submissiondate

// Step 2: Use by prefix to group observations by R_mor_village
bys R_Cen_village_str: egen unique_dates = total(submissiondate != submissiondate[_n-1])

// Step 3: List the village-wise total number of unique dates
list R_Cen_village_str unique_dates

 *Average productivity per day by surveyor                                                          

egen tag = tag(R_Cen_enum_name submissiondate)
egen days_worked = total(tag), by(R_Cen_enum_name)
bys R_Cen_enum_name: gen total_surveys_done_enum = _N
gen daily_avg_enum = round(total_surveys_done_enum/days_worked, .01) 
egen enum_tag = tag(R_Cen_enum_name)
