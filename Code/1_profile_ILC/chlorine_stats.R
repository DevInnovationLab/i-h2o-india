...
# Final graph with the footnote 

#######################################################

df.tap.filtered <- df.tap %>%
  filter(chlorine_concentration <= 2.5)
df.vil.cl.less <- df.tap
df.vil.cl.less$Date <- mdy(df.vil.cl.less$Date)
...
for (i in village_list) {
  ...
  df.vil.cl <- df.tap.filtered %>% filter(village == i)
  ...
  tiu <- ggplot(df.vil.cl, aes(x = Date, y = chlorine_concentration, color = Distance, shape = Test, group = 1)) +
    ...
    scale_y_continuous(limits = c(0.00, 2.50), breaks = seq(0, 2.5, by = 0.1)) + # Increased y-axis limit to 2.5
    theme(
      legend.text = element_text(size = 8), # Increased legend text size
      legend.key.size = unit(0.5, "cm"),   # Reduce legend key size
      legend.box = "vertical",       # Change legend box to vertical
    ...
    if (any(df.vil.cl.less$chlorine_concentration[df.vil.cl.less$village == i] > 2.5)) {
      annotate("text", x = max(df.vil.cl.less$Date[df.vil.cl.less$village == i], na.rm = TRUE), y = 2.5, label = paste0("Note: ", sum(df.vil.cl.less$chlorine_concentration[df.vil.cl.less$village == i] > 2.5), " values > 2.5"), hjust = 1)
    }
...
df.new.GV.filtered.stored <- df.new.GV.stored %>%
  filter(chlorine_concentration <= 2.5)
GV_village_list <- unique( df.new.GV.stored$village) 
View(GV_village_list)
df.new.GV.less <- df.new.GV.stored
...
for (i in GV_village_list) {
  ...
  file_path <- paste0("C:/Users/Archi Gupta/Box/Data/99_Archi_things in progress/GV_Stored_Village", i, ".png")
