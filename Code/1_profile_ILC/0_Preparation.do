...
clear all
	tempfile gsheet
	copy "https://docs.google.com/spreadsheets/d/e/2PACX-1vSyzYHdQBmMN3Kq9Dj2ZTFt66FWwfpBlX11jeb25nKGkttV0OQ_XWl9fqbAhavBzm--Rey0KTYx0fzw/pub?output=xlsx" `gsheet', replace
	import excel using `gsheet', first sheet("Village codes") clear
	keep if Selected=="Selected"
	gen Treat_V=.
	replace Treat_V=1 if assignment=="T"
	replace Treat_V=0 if assignment=="C"
	rename (Panchayat Villagecodes Blockcode) (Panchatvillage village BlockCode)
	gen village_IDinternal=string(village)
* use "${DataOther}India ILC_Pilot_Rayagada Village Tracking_1.dta",clear
* replace village_IDinternal="20101" if Village=="Badabangi"

* Adding Additional cleaning needed
merge 1:1 village_IDinternal using "${DataOther}India ILC_Pilot_Rayagada Village Tracking_2.dta", keep(1 3)
* Adding distance variable
merge 1:1 village_IDinternal using "${DataOther}India ILC_Pilot_Rayagada Village Tracking_3.dta", keep(1 3) nogen
rename  HHchoicecriteria V_Num_HH
drop District Selected R S T U V W X Y Z AA AB AC AD AE AF
order assignment Treat_V, first 
