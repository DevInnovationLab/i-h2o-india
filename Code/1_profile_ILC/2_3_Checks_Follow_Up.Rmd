...
resp_available <- df.temp %>%  filter(R_FU_resp_available == 1 & R_FU_replacement == 0 ) %>% 
  ...
  group_by(R_FU_r_cen_village_name_str) %>% mutate("Original \ Respondent \ Found" =n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU_r_cen_village_name_str)
resp_notavailable <- df.temp %>%  filter(R_FU_resp_available != 1  ) %>% 
  select(R_FU_r_cen_village_name_str) %>%
  group_by(R_FU_r_cen_village_name_str) %>% mutate("Respondent \ NOT Found" =n()) %>% ungroup()  %>% unique() %>% 
  arrange(R_FU_r_cen_village_name_str)
...
xtab<-xtable(tab, caption= "Distribution of Surveys by Date and Village")
print(xtab,file= paste0(overleaf(), "Table/Table_survey_by_date_village.tex"),append=T,table.placement = "h", caption.placement="bottom", hline.after=seq(from=-1,to=nrow(xtab),by=1))
...
xtab<-xtable(tab, caption= "Distribution of Surveys by Enumerator, Date and Village")
print(xtab,file= paste0(overleaf(), "Table/Table_survey_by_enum_date_village.tex"),append=T,table.placement = "h", caption.placement="bottom", hline.after=seq(from=-1,to=nrow(xtab),by=1))
...
resp_hh_location_dur_enum <- df.temp.consent %>%
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Locating Respondent" = round(mean(FU_locatehh_dur_min),2)) %>% ungroup()
consent_dur_enum <- df.temp.consent %>%
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Consent" = round(mean(FU_consent_dur_min),2)) %>% ungroup()
secA_dur_enum <- df.temp.consent %>% 
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Sec A: WASH Access" = round(mean(FU_secA_dur_min),2)) %>% ungroup()
secB_dur_enum <- df.temp.consent %>% 
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Sec B: Government Tap" = round(mean(FU_secB_dur_min),2))  %>% ungroup()
secC_dur_enum <- df.temp.consent %>% 
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Sec C: Chlorination Perceptions" = round(mean(FU_secC_dur_min),2))%>% ungroup()
secD_dur_enum <- df.temp.consent %>% 
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Sec D: Burden" = round(mean(FU_secD_dur_min),2))  %>% ungroup()
secE_dur_enum <- df.temp.consent %>% mutate(FU_secE_dur_min  = ifelse(FU_secE_dur_min<0 , NA, FU_secE_dur_min)) %>% 
  group_by(R_FU_enum_name_label) %>%
 dplyr:: summarise("Sec E: Water Quality Testing"= round(mean(FU_secE_dur_min, na.rm = T),2))  %>% ungroup()
...
stargazer(df.duration.enum, summary=F, title= "Duraction by section Baseline HH Survey by enumerator",float=F,rownames = F,
           covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Duration_enum_HH_Survey.tex"))
...
resp_hh_location_dur <- df.temp.consent %>%
 dplyr:: summarise(mean = mean(FU_locatehh_dur_min), min = min(FU_locatehh_dur_min), max = max(FU_locatehh_dur_min))  %>%
  mutate(variable = "Locating HH")
consent_dur <- df.temp.consent %>% 
 dplyr:: summarise(mean = mean(FU_consent_dur_min), min = min(FU_consent_dur_min), max = max(FU_consent_dur_min) ) %>%
  mutate(variable = "Consent")
secA_dur <- df.temp.consent %>% 
 dplyr:: summarise(mean = mean(FU_secA_dur_min), min = min(FU_secA_dur_min), max = max(FU_secA_dur_min) ) %>%
  mutate(variable = "Sec A: WASH Access")
secB_dur <- df.temp.consent %>% 
 dplyr:: summarise(mean = mean(FU_secB_dur_min), min = min(FU_secB_dur_min), max = max(FU_secB_dur_min) ) %>%
  mutate(variable = "Sec B: Government Tap")
secC_dur <- df.temp.consent %>% 
 dplyr:: summarise(mean = mean(FU_secC_dur_min), min = min(FU_secC_dur_min), max = max(FU_secC_dur_min) ) %>%
  mutate(variable = "Sec C: Chlorination Perceptions")
secD_dur <- df.temp.consent %>% 
 dplyr:: summarise(mean = mean(FU_secD_dur_min), min = min(FU_secD_dur_min), max = max(FU_secD_dur_min) ) %>%
  mutate(variable = "Sec D: Burden")
secE_dur <- df.temp.consent %>% mutate(FU_secE_dur_min  = ifelse(FU_secE_dur_min<0 , NA, FU_secE_dur_min)) %>% 
 dplyr:: summarise(mean = mean(FU_secE_dur_min, na.rm = T), min = min(FU_secE_dur_min,  na.rm = T), max = max(FU_secE_dur_min,  na.rm = T) ) %>%
  mutate(variable = "Sec E: Water Quality Testing")

df.duration <- bind_rows(resp_hh_location_dur,consent_dur, secA_dur,secB_dur, secC_dur, secD_dur, secE_dur)
...
stargazer(chlorine_check, summary=F, title= "Chlorine reading greater than 0.10",float=F,rownames = F,
...
stargazer(df.append, summary=F, title= "WASH Section Summary",float=F,rownames = F,
           covariate.labels=NULL, out=paste0(overleaf(),"Table/Table_Wash_HH_Survey.tex"))

```


```{r descriptive sec B wash access}
df.tap<- df.temp.consent %>% select(R_FU_water_source_prim, R_FU_water_sec_yn,  R_FU_quant, R_FU_water_treat, R_FU_water_stored)
#assign labels for each
