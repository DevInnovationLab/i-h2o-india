...
#install.packages("googledrive", "googlesheets4", "readxl", "dplyr", "lubridate", "haven", "expss", "stargazer")
...
overleaf <- function() {
  user <- Sys.info()["user"]
  if (user == "asthavohra") {
    overleaf = "/Users/asthavohra/Dropbox/Apps/Overleaf/Everything document -ILC/Table/"
  } 
  else if (user==""){
    overleaf = ""
  } 
...
consented <- df.temp.consent %>% select(R_FU_r_cen_village_name_str) %>%
...
#output to tex 
 stargazer(df.progress, summary=F, title= "Overall Progress: Baseline HH Survey",float=F,rownames = F,
           covariate.labels=NULL, out=paste0(overleaf(),"/Table_Progress_HH_Survey.tex"))
