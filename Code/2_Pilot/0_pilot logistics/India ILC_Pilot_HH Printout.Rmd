...
#Jeremy's file path
sheet_path <- "C:/Users/jerem/Box/India Water project/2_Pilot/Data/1_raw/0_field logistics/India ILC_Pilot_Household Tracking.xlsx" 


#Reading in raw data
hh <- read_excel(sheet_path, sheet = 3)
...
hh_randomization <- read_excel(sheet_path, sheet = 1)%>%
 dplyr::select(household_ID,
               selected_for_baseline_survey,
               selected_for_baseline_WQ_sample,
               selected_for_follow_up_1,
...
hh <- hh%>%
  ...
  dplyr::select(household_ID, district, block, village, village_ID, assignment,
...
hh_printout%>%
  ...
  add_header_above(c("Baseline Survey HH Information " = 14)) %>%
