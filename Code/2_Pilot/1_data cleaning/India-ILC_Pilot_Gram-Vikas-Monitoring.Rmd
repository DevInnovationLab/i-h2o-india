...
weekly_plan <- tibble(Date = date(visit_date),
       Day = c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday",
               "Saturday", "Sunday"),
       Plan = c("Asada", #Needs to be manually updated
                "Asada and Gopi Kankubadi",
                "Asada, Mukundpur and Gopi Kankubadi",
                "Asada and Mukundpur",
                "Gopi Kankubadi, Mukundpur and Tandipur",
                "Tandipur and Gopi Kankubadi",
                "Flexible Day")
       )
...
#Calculating average # of days between refills per village
gv_refill_days <- gv_refill%>%
  dplyr::select(Village = Village, 
         Date = `Date:`, 
         Refilled = `Did you/the pump operator refill chlorine tablets since the last visit?`) %>%
  mutate(Date = ymd(Date)) %>% # Parse Date column to Date format
  filter(tolower(Refilled) == "yes") %>% # Filter rows where chlorine tablets were refilled
  arrange(Village, Date) # Sort data by Village and Date

gv_refill_days <- gv_refill_days%>%
  group_by(Village) %>%
  mutate(Days_Between_Refills = as.numeric(difftime(Date, lag(Date), units = "days"))) %>%
  ungroup()

# Calculate the average number of days between refills for each village
average_days_per_village <- gv_refill_days%>%
  group_by(Village) %>%
  summarize(Average_Days = mean(Days_Between_Refills, na.rm = TRUE)) %>%
  drop_na() # Remove villages with no valid data

average_days_per_refill <- gv_refill_days%>%
  summarize(Average_Days = mean(Days_Between_Refills, na.rm = TRUE),
            refill_sd = sd(Days_Between_Refills, na.rm = TRUE)) %>%
  drop_na()

# View the results
#print(average_days_per_village)
