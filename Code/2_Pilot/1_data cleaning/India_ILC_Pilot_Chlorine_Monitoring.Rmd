...
long <- read_csv("C:/Users/jerem/Box/India Water project/2_Pilot/Data/3_final/1_15_Longitudinal Testing.csv")


#Niharika's file path
#mon <- read_csv("/Users/uchicago/Library/CloudStorage/Box-Box/India Water project/2_Pilot/Data/1_raw/Daily Chlorine Monitoring Form_WIDE.csv")

#mon_full <- read_csv("/Users/uchicago/Library/CloudStorage/Box-Box/India Water project/2_Pilot/Data/1_raw/india_ilc_pilot_monitoring_WIDE.csv")
...
user_path <- function() {
  ...
  if (user == "asthavohra") { 
    ...
    path = "/Users/uchicago/Library/CloudStorage/Box-Box/India Water project/2_Pilot/Data/"
		overleaf = "/Users/uchicago/Dropbox/Apps/Overleaf"
  } 
...
#Manual Correction (GitHub issue #95): Correcting the entries of chlorine readings for one obs
mon <- mon %>%
  mutate(first_nearest_tap_fc = ifelse(SubmissionDate == "10-Feb-2024 11:06:10" & village_ID == "50501", 0.80, first_nearest_tap_fc))
...
#Niharika to Jeremy: chnaging the format from mdy_hms to dmy_hms based on raw data downloaded on Oct 3, 2024
#Getting date of test
mon$valve_open_time <- dmy_hms(mon$valve_open_time)
mon <- mon%>%
  mutate(test_date = date(valve_open_time))

mon_full$valve_open_time <- dmy_hms(mon_full$valve_open_time)
mon_full <- mon_full%>%
  mutate(test_date = date(valve_open_time))
...
```{r JPAL Chlorine Monitoring results - pt 2, message = FALSE, fig.height=6, fig.width=12}
...
# Anonymizing village names and filter for Naira and Mukundpur for Odisha Report
mon_wide_anonymized <- mon_wide %>%
  mutate(village_name = case_when(
    village_name == "Naira" ~ "NAI",  
    village_name == "Mukundpur" ~ "MU",  
    TRUE ~ village_name  
  )) %>%
  filter(village_name %in% c("NAI", "MU")) %>%  
  filter(test_date > "2024-04-05") %>% 
  filter(test_date < "2024-10-30") %>% 
  filter(!is.na(chlorine_concentration))  # Remove rows with NA 
...
# Recode chlorine test sample types
mon_wide_anonymized$chlorine_test <- factor(mon_wide_anonymized$chlorine_test)
mon_wide_anonymized$chlorine_test <- fct_recode(mon_wide_anonymized$chlorine_test,
                                                "Nearest Tap" = "Nearest Tap - Free Chlorine",
                                                "Farthest Tap" = "Farthest Tap - Free Chlorine")
  ...
  p1 <- ggplot(data = mon_wide) + 
    ...
    geom_text(data = filter(mon_wide, village_name == "Asada"), 
            aes(x = as.Date("2024-04-25"), y = 1.2), 
            label = "Target Range", angle = 0, vjust = -0.5, color = "red", size = 3)
  
  print(p1)


  p2 <- ggplot(data = mon_wide_anonymized) + 
  geom_point(aes(x = test_date, y = chlorine_concentration, color = chlorine_test)) +
  geom_line(aes(x = test_date, y = chlorine_concentration, color = chlorine_test)) +
  scale_color_manual(values = c("Nearest Tap" = "blue", 
                                "Farthest Tap" = "#FF8C00")) +  # Set specific colors
  scale_y_continuous(
    limits = c(0, 1.2),
    expand = c(0, 0)
  ) +
  labs(x = "Month", 
       ...
       color = "Tap Location",
       title = "Chlorine Monitoring Results across the Distribution Line") +
  theme_bw() + 
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  facet_wrap(~village_name, ncol = 3)   # Facet by the anonymized village names (NAI, MU)
...
# Longitudinal Chlorine Testing Over Supply Time


```{r long testing over supply time}

cutoff_date <- as.Date("2024-09-04")


# Filter out chlorine values (tw_fc) below 0.2
filtered_data <- long%>%
...
# Calculate the mean, standard deviation, and range for each village
village_stats <- filtered_data %>%
  group_by(village_name) %>%
  summarise(
    chlorine_mean = mean(tw_fc, na.rm = TRUE),
    chlorine_sd = sd(tw_fc, na.rm = TRUE),
    chlorine_range = max(tw_fc, na.rm = TRUE) - min(tw_fc, na.rm = TRUE)
  ) %>%
  arrange(desc(chlorine_range))

# Display the statistics for each village
print(village_stats)

# Calculate the overall mean and standard deviation of chlorine values
overall_stats <- filtered_data %>%
  summarise(
    overall_mean = mean(tw_fc, na.rm = TRUE),
    overall_sd = sd(tw_fc, na.rm = TRUE)
  )

# Display the overall statistics
print(overall_stats)

# Extract the statistics specifically for Asada village
asada_stats <- village_stats %>%
  filter(village_name == "Asada")

# Display the statistics for Asada village
print(asada_stats)


# Calculate paired ranges between nearest and farthest taps based on tw_fc_variable
paired_ranges <- filtered_data %>%
  group_by(village_name, tw_fc_variable) %>%
  summarise(
    nearest_value = tw_fc[location == "Nearest Tap"],
    farthest_value = tw_fc[location == "Farthest Tap"],
    range = abs(farthest_value - nearest_value)
  ) %>%
  ungroup()

# Find the greatest paired range for each village
village_paired_ranges <- paired_ranges %>%
  group_by(village_name) %>%
  summarise(
    max_paired_range = max(range, na.rm = TRUE)
  ) %>%
  arrange(desc(max_paired_range))

#Average range overall
village_paired_ranges_overall <- paired_ranges%>%
  summarise(avg_range = mean(range),
            sd_range = sd(range))


# Display the paired ranges for each village
print(village_paired_ranges)

# Extract the greatest paired range and corresponding village
greatest_paired_range <- village_paired_ranges %>%
  filter(max_paired_range == max(max_paired_range, na.rm = TRUE))

# Display the village with the greatest paired range
print(greatest_paired_range)
```
